...
class ESRGAN:
    ...
    def enhance_single_image(self, input_path, output_path):
        ...
        img = enhance_fluorescence(img)  # **先增亮細胞**
...
# ====== OpenCV 螢光增強 ======
def enhance_fluorescence(image, gamma=1.5):
    # 1️⃣ Gamma 校正
    invGamma = 1.0 / gamma
...
class CellCounterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("細胞計數 YOLO - Green + ESRGAN")
    ...
    def run_analysis(self):
        ...
        image_np = cv2.cvtColor(np.array(enhanced_img), cv2.COLOR_RGB2BGR)

        # 再做 YOLO
        if os.path.exists(result_file):
            annotated = cv2.imread(result_file)
            cell_count = len(model(annotated)[0].boxes.xyxy.cpu().numpy())
        else:
