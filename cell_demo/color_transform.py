...
# 使用 CLAHE 方法增強圖像對比度
clahe = cv.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))# 創建 CLAHE 物件
def enhance_contrast(input_path, output_path): 
    image = cv.imread(input_path) #讀取圖像
    if image is None:
        print(f"❌ 無法讀取圖像：{input_path}")
        return
    #gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)  # 將圖像轉為灰階 (測試先轉灰階在增強對比度 效果?)
    ...
    # clahe.setClipLimit(8.0)  # 調整對比度限制
...
if __name__ == "__main__":
    # 設定來源資料夾與輸出資料夾路徑
    INPUT_FOLDER = r"D:\cell\opencv_trans\origin_ph\\"    # 換成你的圖片資料夾路徑
    OUTPUT_FOLDER = r"D:\cell\opencv_trans\output_ph\\"     # 轉換後的輸出資料夾
